...
class BD{
	...
	function stmt_bind_par(&$stmt, $params){
		...
		foreach ($params as $key => $p) {
			$paramsRef[] = &$params[$key]; // pass by reference, not value
                ...
                //echo'<br>bind ';print_r($paramsRef);
	...
	function arr_to_str($arr, $del = ', '){
		...
		return array($str1, $str2, $str3, $vals);
	}
	function insert_on_update($table, $arr){
		$str = $this->arr_to_str($arr);
		$q = "INSERT INTO `".$table."`(".$str[0].") VALUES (".$str[1].")  ON DUPLICATE KEY UPDATE ".$str[2];
		//echo'<br>iou ';print_r($str);
		return $this->sql($q, array_merge($str[3], $str[3]));
	}
