...
$collPost = $bd->get_row("SELECT count(*) as `coll` FROM `users` LEFT JOIN `social` ON `users`.`id`=`social`.`id_user` WHERE `social`.`post`='yes'");
?>
<div class="gcanvas">
		<div class="title">
                    <img id="title_pdd" alt="Правила дорожного движения" src="../images/pdd.png"/>
<!--<a  href="http://<? echo $hh?>/index.php?a=page&id=<? echo $index['index'];?>">
Правила дорожного движения</a>-->
                </div>
		<div class="header">
		  <div class="logo"><a title="Главная <? echo $collus['coll'],' (',$collPost['coll']?>)" href="http://<? echo $hh?>"> </a></div>
		  <div class="menu">
