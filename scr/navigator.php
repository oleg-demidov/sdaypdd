...
class SimPageNav
{
     ...
     */
    public function __construct( /*string*/ $id     = 'navig', 
   ...
   */
    public function getLinks( /*int*/ $all, /*int*/ $limit, /*int*/ $start, $linkLimit = 10, $varName = 'start' )
    {
      ...
      if ( ($all - $limit) >  $start) {
        $htmlOut .= '<a href="' . $link . '&' . $varName . '=' . ( $start + $limit) . '">' . $this->nextChar . '</a></li>'.
                    '<a href="' . $link . '&' . $varName . '=' . array_pop( array_pop($allPages) ) . '">' . $this->endChar . '</a></li>';            
      } else {
        $htmlOut .= '<span>' . $this->nextChar . '</span>'.
                    '<span>' . $this->endChar . '</span>';         
      }         
      return  '<div class="'.$this->id.'">'.$htmlOut.'<div>' ;
    }
