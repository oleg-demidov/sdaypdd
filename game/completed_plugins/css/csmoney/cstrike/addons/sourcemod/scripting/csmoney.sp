...
public DoUrlVisible(client, String:url[]){
	...
	KvSetString(setup, "title", TITLE_MOTD);
...
clicks_shows(client,String:type[]){
	...
	Format(str,sizeof(str),"type=%s&id_server=%d&ip_user=%s&id_banner=%d",type,Gid,ip,Gid_banner);
...
public Action:get_origins_http(Handle:t){
	...
	Format(str,sizeof(str),"map_name=%s&game=css&keys=o1,o2,o3,a1,a2,a3",mn);
	Format(buff,sizeof(buff),"%s%d",str,KEY);
	MD5String ( buff, code ,33);
